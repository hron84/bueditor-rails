...
E.tagDialog = function(tag, fields, opt) {
  ...
  var dopt = $.extend({title: 'Tag editor - @tag', {'@tag': tag.toUpperCase(}), stitle: 'OK', validate: false, submit: function(a, b) {return E.tgdSubmit(a, b)}, effect: 'show'}, opt);
...
var fproc = function(f, obj, sel) {
  ...
  f.value = typeof obj.attributes[f.name] == 'string' ? obj.attributes[f.name] : (f.value || '');
  ...
  if (f.required) {
    ...
    f.attributes['title'] = 'This field is required.';
  }
  return f;
};
