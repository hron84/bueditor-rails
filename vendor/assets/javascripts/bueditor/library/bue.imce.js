...
$(function() {I.url =  typeof(Drupal) !== 'undefined' ?  Drupal.settings.BUE.imceURL : ''});

//IMCE button html to be used in forms. Target field's name is required.
I.button = function(fname, text) {
  return I.url ? '<input type="button" id="bue-imce-button" name="bue_imce_button" class="form-submit" value="'+ (text || 'Browse') +'" onclick="BUE.imce.open(this.form.elements[\''+ fname +'\'])">' : '';
};
...
I.open = function(opt) {
  ...
  //Load IMCE once and for all. Run window.bueImceLoad which then runs the ready method.
    ...
    I.pop = BUE.createPopup('bue-imce-pop', 'File Browser', '<iframe src="'+ url +'" frameborder="0"></iframe>');
    I.setPos();
  }
};
...
window.bueImceLoad = function(win) {
  (I.win = win).imce.setSendTo('Send to editor', I.finish);
  I.ready(win, I.pop);
