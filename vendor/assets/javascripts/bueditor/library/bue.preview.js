...
E.prv = function(safecheck) {
  ...
  if (safecheck && !(E.safeToPreview = E.safeToPreview || content.indexOf('<') == -1)) {
    content = '<div class="warning">' + 'The preview is disabled due to previously inserted HTML code in the content. This aims to protect you from any potentially harmful code inserted by other editors or users. If you own the content, just preview an empty text to re-enable the preview.' + '</div>';
  }
...
//Ajax preview. Requires ajax_markup module.
 E.prvAjax = function(format, callback) {
  ...
  if (!($xM = $.ajaxMarkup)) {
    return E.prvShow('Preview requires <a href="http://drupal.org/project/ajax_markup">Ajax markup</a> module with proper permissions set.');
  }
