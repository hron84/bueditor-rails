...
E.find = function (str, matchcase, regexp) {
  ...
  if (index == -1) {
    if (from == 0) {
      alert('No matching phrase found!');
    }
    else if (confirmEOT()) {
      E.makeSelection(0, 0);
...
E.replace = function(str1, str2, matchcase, regexp) {
  ...
  if (found && confirm('Replace this occurance of "!text"?', {'!text': s})) {
...
E.replaceAll = function(str1, str2, matchcase, regexp) {
  ...
  alert('Total replacements: !count', {'!count': n});
  return E;
};
...
E.frForm = function() {
  ...
  BUE.frPop.open(opt.title || (opt.isrep ? 'Find & Replace' : 'Search'));
...
var confirmEOT = function() {
  return confirm('End of textarea reached. Continue search at the beginning of textarea?');
};
...
var theForm = function () {
  ...
  F += Dv(Cb('matchcase', 'Match case') +' '+ Cb('regexp', 'Regular expressions'));
  F += Dv(Bt('findbutton', 'Find next') +' '+ Bt('replacebutton', 'Replace') +' '+ Bt('replaceallbutton', 'Replace all'));
